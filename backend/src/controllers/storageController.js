...
const getOverview = async (req, res) => {
  try {
    ...
    const recycleBinClients = await ClientModel.countDocuments({ deletedAt: { $exists: true } });
    const recycleBinContent = await ContentModel.countDocuments({ deletedAt: { $exists: true } });
    const recycleBinCount = recycleBinClients + recycleBinContent;

    const lastBackup = await ActivityLog.findOne({ type: 'Backup Export' }).sort({ createdAt: -1 });
